...
import 'smoke_weed_screen.dart';

class SmokeTobaccoScreen extends StatefulWidget {
  const SmokeTobaccoScreen({super.key});

  @override
  State<SmokeTobaccoScreen> createState() => _SmokeTobaccoScreenState();
}

class _SmokeTobaccoScreenState extends State<SmokeTobaccoScreen> {
  ...
  Future<void> _saveSmokingStatusAndContinue() async {
    ...
    try {
      await _profileService.upsertProfile(
        ...
        smokingStatusVisible: _visibleOnProfile,
      );

      if (!mounted) return;
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const SmokeWeedScreen()),
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to save smoking status: $e')),
        );
      }
    } finally {
