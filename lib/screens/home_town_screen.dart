...
class _HomeTownScreenState extends State<HomeTownScreen> {
  ...
  void initState() {
    ...
    // Add listener to rebuild on text change to update button state
  ...
  Future<void> _saveHomeTownAndContinue() async {
    final hometown = _controller.text.trim();
    if (hometown.isEmpty || _currentUser == null || _loading) return;
    ...
    try {
      await _profileService.upsertProfile(
        ...
        hometown: hometown,
        hometownVisible: _visibleOnProfile,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to save home town: $e')),
        );
      }
    } finally {
